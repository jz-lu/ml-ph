    ...
    def __block_l2(self, DM_intras, DM_inter):
        ...
        def enforce_acoustic_sum_rule():
            ...
            for i in range(0, l0sz, 3): # intralayer1 and interlayer12
                DM_intras[0][i:i+3,i:i+3] -= sum([sum([sqrt(M[1,j//3]/M[0,i//3]) * blk[i:i+3,j:j+3] for j in range(0,blk.shape[1],3)]) for blk in self.off_diag_blocks])
            for i in range(0, l0sz, 3): # intralayer2 and interlayer 21
                DM_intras[1][i:i+3,i:i+3] -= sum([sum([sqrt(M[0,j//3]/M[1,i//3]) * blk.conjugate().T[i:i+3,j:j+3] for j in range(0,blk.shape[0],3)]) for blk in self.off_diag_blocks])
