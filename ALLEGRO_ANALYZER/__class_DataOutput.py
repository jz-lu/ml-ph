...
class DataOutput:
    ...
    def __init__(self, out_dir, plot_list, cob_matrix):
        print("Initalizing DataOutput object. Output to be stored in %s"%(out_dir))
        ...
        self.__plot_X = np.array([i[0] for i in self.__shifts])
        self.__plot_Y = np.array([i[1] for i in self.__shifts])
    ...
    def plot_e_vs_b(self):
        fig, ax = plt.subplots()
        cf = ax.tricontourf(self.__plot_X, self.__plot_Y, self.__energies, levels=NUM_LEVELS, cmap="RdGy")
    ...
    def plot_z_vs_b(self):
        fig, ax = plt.subplots()
        cf = ax.tricontourf(self.__plot_X, self.__plot_Y, self.__zspacings, levels=NUM_LEVELS, cmap="twilight_shifted")
        fig.colorbar(cf, ax=ax)
        ax.set_xlabel(r"$b_x$")
        ax.set_ylabel(r"$b_y$")
