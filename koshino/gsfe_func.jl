...
function gradient_GSFE(X::Array{Float64,2}, bl::Bilayer)
    ...
    for i=1:size(X,2)
      @inbounds G[1,i] = -c1 * (sin(s[i]) + sin(s[i]+t[i])) +
                 ...
                 -2*c3 * (sin(2. *s[i]) + sin(2. *(s[i]+t[i]))) +
                 c4 * (cos(s[i]) - cos(s[i] + t[i])) +
                 2*c5 * (cos(2*s[i] + 2*t[i]) - cos(2*s[i]))
      @inbounds G[2,i] = -c1 * (sin(t[i]) + sin(s[i]+t[i])) +
                  -c2 * (2. *sin(s[i]+2. *t[i]) + sin(t[i]-s[i]) + sin(2. *s[i]+t[i])) +
                  -2*c3 * (sin(2. *t[i]) + sin(2. *(s[i]+t[i]))) +
                  c4 * (cos(t[i]) - cos(s[i] + t[i])) +
                  2*c5 * (cos(2*s[i] + 2*t[i]) - cos(2*t[i]))
    end
